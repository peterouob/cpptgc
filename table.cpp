...
void Table::add(void* ptr, const std::size_t size, const Flags flags,
                const Destructor dtor) {
  assert(ptr != nullptr && "Table::add: ptr must not be null");

  if (nslots() == 0 || static_cast<double>(nitems_ + 1) >
                           static_cast<double>(nslots()) * load_factor_)
    grow();

  assert(ptr != nullptr && "Table::add: ptr must not be null");
  insert_no_grow(Entry{ptr, size, hash(ptr), flags, dtor});
...
void Table::rehash(const std::size_t new_nSlots) {
  for (const std::vector<Entry> old =
           std::exchange(items_, std::vector<Entry>(new_nSlots));
       auto& e : old) {
    if (!e.empty()) insert_no_grow(e);
  }
}

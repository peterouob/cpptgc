...
void check_invariants(const TestTable& t) {
  ...
  if (n != 0) {
    ...
    ASSERT_LE(static_cast<double>(t.size()),
              static_cast<double>(n) * kLoadFactor);
  }

  const auto occupied =
      std::ranges::count_if(slots, [](const Entry& e) { return !e.empty(); });
  ...
  for (std::size_t i = 0; i < n; ++i) {
    ...
    for (std::size_t k = 0; k < dist; ++k) {
      ASSERT_FALSE(slots[(home + k) & (n - 1)].empty())
          << "gap before slot" << i;
    }

    const std::size_t j = (i + 1) & (n - 1);
    if (!slots[j].empty()) {
      ASSERT_LE(t.probe(j, slots[j].hash), dist + 1)
          << "robin hood violated at slot" << i;
    }
